...
* `clipPath` -  CSS свойство clip-path создаёт ограниченную область, которая определяет какая часть элемента должна быть видимой. Те части, которые находятся внутри области, видимы, в то время как части вне области, скрыты.
* `{ y: 0, opacity:1 ,delay: 2, stagger: 0.1, ease: "back.out(3)" }`
  ...
  > 
  > `stagger` можкт быть отрицательным - то же самое, но в обратном порядке, так что последний элемент будет начинаться первым.
...
```
### 💡 Смещение элемента
> Если элемент в конце уменьшается, то он сместится от левого края, т.к. уменьшение идёт от его центра (`left: 0` не поможет)
>
